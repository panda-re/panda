...
void tubtf_open(char *filename, TubtfColw colw) {
  ...
  fseek(tubtf->fp, header_size, SEEK_SET);
}
...
void tubtf_write_el_32(uint32_t cr3, uint32_t eip, uint32_t type, uint32_t arg1, uint32_t arg2, uint32_t arg3, uint32_t arg4)  {
  ...
  fwrite(&(cr3),  sizeof(cr3),  1, tubtf->fp);
  fwrite(&(eip),  sizeof(eip),  1, tubtf->fp);
  fwrite(&(type), sizeof(type), 1, tubtf->fp);
  fwrite(&(arg1), sizeof(arg1), 1, tubtf->fp);
  fwrite(&(arg2), sizeof(arg2), 1, tubtf->fp);
  fwrite(&(arg3), sizeof(arg3), 1, tubtf->fp);
  fwrite(&(arg4), sizeof(arg4), 1, tubtf->fp);
  tubtf->num_rows ++;
}
...
void tubtf_close(void) {
  ...
  fseek(tubtf->fp, sizeof (tubtf->version) + sizeof(tubtf->colw) + sizeof(tubtf->contents_bits), SEEK_SET);
  printf ("%d rows in trace\n", tubtf->num_rows);
  fwrite(&(tubtf->num_rows), sizeof(tubtf->num_rows), 1, tubtf->fp);
  fclose(tubtf->fp);
}
