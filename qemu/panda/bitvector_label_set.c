...
//#define LST_DUNNO 0
//#define LST_COPY 1
// implicitly, anything higher than 1 is a compute

//typedef uin32_t LabelSetType;
...
/*
#define MAX_RECYCLED_NUM 1024
static LabelSet *recycled_list[MAX_RECYCLED_NUM];
static uint32_t last_recycled_index = -1;
*/
...
static SB_INLINE LabelSet *labelset_new(void) {  
  ...
  //  if (last_recycled_index == -1) {
      ...
      ls->set = (BitSet *) bitset_new();
      ls->type = LST_DUNNO;
      ls->count = 1;
      /*
  }
  else {
      ls = recycled_list[last_recycled_index];
      ls->type = LST_DUNNO;
      ls->count = 0;
      last_recycled_index--;
  }
...
/*
static SB_INLINE void labelset_recycle(LabelSet *ls) {
    if ((last_recycled_index + 1) < MAX_RECYCLED_NUM) {
        last_recycled_index++;
        recycled_list[last_recycled_index] = ls;
    }
    else {
        //mz recycled list is full
        labelset_free(ls);
    }
}

static SB_INLINE void labelset_free_recycled() {
    for (; last_recycled_index >= 0; last_recycled_index--) {
        labelset_free(recycled_list[last_recycled_index]);
    }
}
*/
...
static SB_INLINE uint8_t labelset_is_compute(LabelSet *ls) {
  return (ls->type > LST_COPY);
}

static SB_INLINE uint8_t labelset_test_type(LabelSet *ls, LabelSetType isItThisType) {
...
static SB_INLINE uint8_t labelset_is_empty(LabelSet *ls) {
  if (ls == 0) return TRUE;
...
static SB_INLINE uint8_t labelset_member(LabelSet *ls, uint32_t l) {
...
static SB_INLINE void labelset_save(void * /* QEMUFile * */ f, LabelSet *ls) {
  qemu_put_be32(f, ls->type);
  bitset_save(f, ls->set);
}

static SB_INLINE void labelset_fill(void * /* QEMUFile * */ f, LabelSet *ls) {
  ls->type = qemu_get_be32(f);
  bitset_fill(f, ls->set);
}


static SB_INLINE LabelSet *labelset_load(void * /* QEMUFile * */ f) {
