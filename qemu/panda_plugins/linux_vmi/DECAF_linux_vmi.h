...
#ifdef DECAF_DEBUG
#include <stdio.h>
#define DECAF_DLOG ptout
extern FILE *DECAF_DLOG;
#endif
...
/**
 * IMPLEMENT_OFFSET_GETTER is a macro for generating uniform inline
 * functions that retrieve data based on a location+offset.
 * Three EXAMPLES of functions generated by the macro:
 *
 * gpid_t DECAF_get_pid(CPUState* env, gva_t task_struct_addr)
 * {
 *   gpid_t pid;
 *   if (DECAF_read_mem(env, task_struct_addr + task_struct_pid_offset, &pid, sizeof(gpid_t)) != 0)
 *   {
 *     return (-1);
 *   }
 *   return pid;
 ...
 * {
 *   target_long tgid;
 *   if (DECAF_read_mem(env, task_struct_addr + task_struct_tgid_offset, &tgid, sizeof(target_long)) != 0)
 *   {
 *     return (-1);
 *   }
 *   return (tgid);
 ...
 * {
 *   gva_t gl;
 *   if (DECAF_read_mem(env, task_struct_addr + task_struct_tgid_offset, &gl, sizeof(gva_t)) != 0)
 *   {
 *     return (-1);
 *   }
 *   return (gl);
