        ...
        if (info.addr > 0 && alloc_ever[cr3].contains(info.addr)) {
            if (!alloc_now[cr3].contains(info.addr)) {
                ...
                for (auto it = ri.valid_ptrs.begin(); it != ri.valid_ptrs.end(); it++) {
                    ...
                    // RW taint pointer into the freed range
                    if (!taint2_enabled()){
                        // XXX HACK can't enable taint in the middle of a BB
                        //taint2_set_enable_taint_flag();
                    }
                    else {
                        //taint2_add_taint_ram_single_label(env, *it, 4, 1);
                    }
                }
                // RW taint entire freed buffer
                if (!taint2_enabled()){
                    // XXX HACK can't enable taint in the middle of a BB
                    taint2_set_enable_taint_flag();
                }
                else {
                    taint2_add_taint_ram_single_label(env, ri.begin,
                        ri.end-ri.begin, 1);
                }
...
bool init_plugin(void *self) {
    ...
    //right_cr3 = panda_parse_ulong(args, "cr3", 0x3F98B380);
