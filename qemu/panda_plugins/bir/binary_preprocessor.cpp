...
int main (int argc, char **argv) {
    ...
    uint32_t ii = 0;
    char fn[1024];
    sprintf (fn, "%s.pp", inv_pfx);
    FILE *fp = fopen(fn, "w");
    fwrite((void *) &tot, sizeof(tot), 1, fp);
    //   timer_start("a");
    // iterate over max-n grams 
    for ( auto mgram : inv.lexicon[inv.max_n_gram].grams ) {        
        /*        
        printf ("mgram = "); 
        spit_gram_hex(mgram, inv.max_n_gram);
        printf ("\n");
        */
        // e.g. if max n gram is 5-gram, 
        // this might be "abcde"
        fwrite((void *) &mgram, sizeof(mgram), 1, fp);
        if ((ii%(tot/10)) == 0) {
            printf ("ii = %d of %d\n", ii, tot);
            printf ("pp.size %d\n", pp.size());
            //            timer_stop("a");
        ...
        */
        ...
        uint32_t new_rowsize = pppqs.size();
        //        assert (rowsize == new_rowsize);
        fwrite ((void *) &rowsize, sizeof(rowsize), 1, fp);
        int i=0;
        for ( auto &kvp : pppqs ) {
            sp[i].ind = kvp.first;
            sp[i].val = log(kvp.second / pg);
            pp.insert(sp[i].val);
            //  printf ("(%d %.3f) ", sp[i].ind, sp[i].val);
            i++;
        }
        // this is precomputed row for mgram
        fwrite((void *) sp, sizeof(Score), rowsize, fp);
    }
