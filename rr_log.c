...
// our debug rr_assert
static inline void rr_assert_fail(const char *exp, const char *file, int line, const char *function) {
...
void rr_record_cpu_mem_unmap(RR_callsite_id call_site,
                                   hwaddr addr, uint8_t *buf, hwaddr len, int is_write) {
    RR_log_entry *item = &(rr_nondet_log->current_item);
    //mz just in case
    memset(item, 0, sizeof(RR_log_entry));
...
void rr_record_cpu_reg_io_mem_region(RR_callsite_id call_site,
                                         hwaddr start_addr, ram_addr_t size, ram_addr_t phys_offset) {
    RR_log_entry *item = &(rr_nondet_log->current_item);
    //mz just in case
    memset(item, 0, sizeof(RR_log_entry));
  ...
  if (rr_nondet_log) {
    if (rr_log_is_empty()) {
      printf ("%s:  log is empty.\n", rr_nondet_log->name);
        ...
        printf("%s:  %10" PRIu64 " (%6.2f%%) instrs. %7.2f sec. %5.2f GB ram.\n",
                name,
...
static void qmp_begin_record_from(const char *snapshot, const char *file_name, Error **errp) {
