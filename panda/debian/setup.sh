...
# First build the container to installer so you get the wheel file
DOCKER_BUILDKIT=1 docker build --target installer -t panda --build-arg BASE_IMAGE="ubuntu:${version}" ../..

# Copy wheel file out of container to host, this also preserves wheel name
...
# Finally, finish building main panda container for the target ubuntu version
