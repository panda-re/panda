...
typedef union panda_cb {
	...
	/* Callback ID: PANDA_CB_BEFORE_BLOCK_EXEC_INVALIDATE_OPT
       ...
       Arguments:
        int *env: the current CPU state
        int *tb: the TB we are about to execute

       Return value:
        true if we should invalidate the current translation block
        and retranslate, false otherwise
    */
    bool (*before_block_exec_invalidate_opt)(int *env, int *tb);
    ...
    /* Callback ID: PANDA_CB_BEFORE_BLOCK_EXEC
       ...
       Return value:
        unused
    */
    int (*before_block_exec)(int *env, int *tb);
    ...
    /* Callback ID: PANDA_CB_AFTER_BLOCK_EXEC
       ...
       Arguments:
        int *env: the current CPU state
        int *tb: the TB we just executed
        int *next_tb: the TB we will execute next (may be NULL)

       Return value:
        unused
    */
    int (*after_block_exec)(int *env, int *tb);
    ...
    /* Callback ID: PANDA_CB_BEFORE_BLOCK_TRANSLATE
       ...
       Arguments:
        int *env: the current CPU state
        int pc: the guest PC we are about to translate

       Return value:
        unused
    */
    int (*before_block_translate)(int *env, int pc);
    ...
    /* Callback ID: PANDA_CB_AFTER_BLOCK_TRANSLATE
       ...
       Arguments:
        int *env: the current CPU state
        int *tb: the TB we just translated
    ...
    */
    int (*after_block_translate)(int *env, int *tb);
    ...
    */
    bool (*insn_translate)(int *env, int pc);
    ...
    /* Callback ID: PANDA_CB_INSN_EXEC
       ...
       Arguments:
        int *env: the current CPU state
        int pc: the guest PC we are about to execute
    ...
    */
    ...
    /* Callback ID: PANDA_CB_AFTER_INSN_TRANSLATE
       ...
       Arguments:
        int *env: the current CPU state
        int pc: the next guest PC we've translated
    ...
    */
    bool (*after_insn_translate)(int *env, int pc);
    ...
    /* Callback ID: PANDA_CB_AFTER_INSN_EXEC
       ...
       Arguments:
        int *env: the current CPU state
        int pc: the next guest PC already executed
    ...
    */
    int (*after_insn_exec)(int *env, int pc);
    ...
    */
    ...
    /* Callback ID: PANDA_CB_MONITOR
       ...
       Arguments:
        void *mon: a pointer to the void
    ...
    */
    int (*monitor)(void *mon, const char *cmd);
    ...
    /* Callback ID: PANDA_CB_VIRT_MEM_BEFORE_READ
       ...
       Arguments:
        int *env: the current CPU state
        int pc: the guest PC doing the read
        int addr: the (virtual) address being read
        int size: the size of the read

       Return value:
        unused
    */
    int (*virt_mem_before_read)(int *env, int pc, int addr, int size);
    ...
    /* Callback ID: PANDA_CB_VIRT_MEM_BEFORE_WRITE
       ...
       Arguments:
        int *env: the current CPU state
        int pc: the guest PC doing the write
        int addr: the (virtual) address being written
        int size: the size of the write
        void *buf: pointer to the data that is to be written

       Return value:
        unused
    */
    int (*virt_mem_before_write)(int *env, int pc, int addr, int size, void *buf);
    ...
    /* Callback ID: PANDA_CB_PHYS_MEM_BEFORE_READ
       ...
       Arguments:
        int *env: the current CPU state
        int pc: the guest PC doing the read
        int addr: the (physical) address being read
        int size: the size of the read

       Return value:
        unused
    */
    int (*phys_mem_before_read)(int *env, int pc, int addr, int size);
    ...
    /* Callback ID: PANDA_CB_PHYS_MEM_BEFORE_WRITE
       ...
       Arguments:
        int *env: the current CPU state
        int pc: the guest PC doing the write
        int addr: the (physical) address being written
        int size: the size of the write
        void *buf: pointer to the data that is to be written
       ...
       Return value:
        unused
    */
    int (*phys_mem_before_write)(int *env, int pc, int addr, int size, void *buf);
    ...
    /* Callback ID: PANDA_CB_VIRT_MEM_AFTER_READ
       ...
       Return value:
        unused
    */
    int (*virt_mem_after_read)(int *env, int pc, int addr, int size, void *buf);
    ...
    /* Callback ID: PANDA_CB_VIRT_MEM_AFTER_WRITE
       ...
       Return value:
        unused
    */
    int (*virt_mem_after_write)(int *env, int pc, int addr, int size, void *buf);
    ...
    /* Callback ID: PANDA_CB_PHYS_MEM_AFTER_READ
       ...
       Return value:
        unused
    */
    int (*phys_mem_after_read)(int *env, int pc, int addr, int size, void *buf);
    ...
    /* Callback ID: PANDA_CB_PHYS_MEM_AFTER_WRITE
       ...
       Return value:
        unused
    */
    int (*phys_mem_after_write)(int *env, int pc, int addr, int size, void *buf);
    ...
    /* Callback ID: PANDA_CB_CPU_RESTORE_STATE
       ...
       Arguments:
        int *env: the current CPU state
        int *tb: the current translation block

       Return value:
        unused
    */
    int (*cb_cpu_restore_state)(int *env, int *tb);
    ...
    */
    ...
    /* Callback ID: PANDA_CB_ASID_CHANGED
       ...
       Arguments:
        int* env: pointer to int
        int oldval: old asid value
        int newval: new asid value

       Return value:
        unused
    */
    int (*asid_changed)(int *env, int oldval, int newval);
    ...
    /* Callback ID:     PANDA_CB_REPLAY_HD_TRANSFER,
       ...
       Return value:
        unused
    */
    int (*replay_hd_transfer)(int *env, uint32_t type, uint64_t src_addr, uint64_t dest_addr, uint32_t num_bytes);
    ...
    */
    int (*replay_before_dma)(int *env, uint32_t is_write, uint8_t* src_addr, uint64_t dest_addr, uint32_t num_bytes);
    ...
    /* Callback ID:     PANDA_CB_REPLAY_AFTER_DMA,
       ...
       Arguments:
        ...
        uint32_t num_bytes:  size of transfer
    */
    int (*replay_after_dma)(int *env, uint32_t is_write, uint8_t* src_addr, uint64_t dest_addr, uint32_t num_bytes);
    ...
    */
    int (*replay_handle_packet)(int *env, uint8_t *buf, int size, uint8_t direction, uint64_t old_buf_addr);
    ...
    /* Callback ID:     PANDA_CB_REPLAY_NET_TRANSFER,
       ...
       *at* the point at which it happened, really.
       ...
       Return value:
        unused
    */
    int (*replay_net_transfer)(int *env, uint32_t type, uint64_t src_addr, uint64_t dest_addr, uint32_t num_bytes);
    ...
    /* Callback ID:     PANDA_CB_REPLAY_SERIAL_RECEIVE,
       ...
       Return value:
        unused
    */
    int (*replay_serial_receive)(int *env, uint64_t fifo_addr,
                                 uint8_t value);
    ...
    /* Callback ID:     PANDA_CB_REPLAY_SERIAL_READ,
       ...
       Return value:
        unused
    */
    int (*replay_serial_read)(int *env, uint64_t fifo_addr,
    ...
    /* Callback ID:     PANDA_CB_REPLAY_SERIAL_SEND,
       ...
       Return value:
        unused
    */
    int (*replay_serial_send)(int *env, uint64_t fifo_addr, uint8_t value);
    ...
    /* Callback ID:     PANDA_CB_REPLAY_SERIAL_WRITE,
       ...
       Return value:
        unused
    */
    int (*replay_serial_write)(int *env, uint64_t fifo_addr,
                               uint32_t port_addr, uint8_t value);
    ...
    */
    void (*after_machine_init)(int *env);
...
extern void *panda_cbs[PANDA_CB_LAST];
...
int panda_parse_ulong(panda_arg_list *args, const char *argname, int defval);
int panda_parse_ulong_req(panda_arg_list *args, const char *argname, const char *help);
int panda_parse_ulong_opt(panda_arg_list *args, const char *argname, int defval, const char *help);
