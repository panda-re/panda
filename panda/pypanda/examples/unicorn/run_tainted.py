...
code = [ # r0 += R1. while true, inc R0. With a nop because I'm bad at shellcoding
    b"\x01\x00\x80\xe0", # add r0, r1
...
def my_init(env):
    ...
    env.env_ptr.regs[registers['R1']] = 0x100
    env.env_ptr.pc = ADDRESS
    panda.taint_label_reg(registers['R1'], 10) # Taint R1 with label 10. Should prop into R0
...
def before_block(env, tb):
    ...
    if block_count == 10:
        ...
        if panda.taint_check_reg(registers['R0']):
            for idx, byte_taint in enumerate(panda.taint_get_reg(registers['R0'])):
                labels = byte_taint.get_labels()
                print(f"Register R0 byte {idx} tainted by {labels}")
