...
use panda::prelude::*;
use panda::mem::{virtual_memory_read,virt_to_phys};
...
fn determine_kaslr_offset(cpu : &mut CPUState){
    ...
    let unshifted_comm_address = init_task_address as target_ulong + task_comm_offset as target_ulong;
    ...
    let mut ptr = start;
    let swapper_searcher: regex::bytes::Regex = Regex::new(r"swapper(/0|\x00\x00)\x00\x00\x00\x00\x00\x00").unwrap();
                ...
                if let Ok(mut res) = virtual_memory_read(cpu, ptr, PAGE_SIZE as usize){
                    ...
                    if let Some(m) = swapper_searcher.find(&res) {
                        let offset_found: target_ulong = if had_overflow {
                            ptr as target_ulong + m.start() as target_ulong - tmp_overflow.len() as target_ulong
                        }else{
...
fn init(_: &mut PluginHandle) -> bool {
    ...
    let filename = "/home/luke/workspace/panda/panda/plugins/osi2/ubuntu:4.4.0-170-generic:32.json.xz";
...
fn current_process_name(cpu: &mut CPUState) -> String{
    ...
    let current_task_ptr = read_little_endian_int(cpu, cur_task.address as target_ulong + cpu_0_offset as target_ulong);
    let comm_data = virtual_memory_read(cpu, current_task_ptr as target_ulong + comm_offset as target_ulong, 16).unwrap();
