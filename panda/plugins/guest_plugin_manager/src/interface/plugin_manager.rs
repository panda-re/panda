...
fn get_channel_from_name(origin_channel: ChannelId, buffer: Vec<u8>) {
    ...
    if let Some(channel_id) =
        super::channels::get_channel_from_name(&plugin_name)
    {
        println!(
            "success got channel from name {} FD is {}",
            plugin_name, channel_id
        );
...
extern "C" fn read_callback(channel_id: ChannelId, ptr: *const u8, len: usize) {
    ...
    match ManagerOp::try_from(op_num as usize) {
        Ok(ManagerOp::GetChannelFromName) => {
            get_channel_from_name(channel_id, buf)
        }
