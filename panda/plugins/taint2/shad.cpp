...
FastShad::FastShad(std::string name, uint64_t labelsets) : Shad(name, labelsets)
{
    ...
    if (labelsets < (1UL << 24)) {
        ...
        printf("taint2: Allocating [%s] small fast_shad (%" PRIu64 " bytes) using malloc @ %lx.\n",
               name.c_str(), bytes, (uint64_t)array);
        assert(array);
        memset(array, 0, bytes);
    } else {
        printf("taint2: Allocating [%s] large fast_shad (%lu bytes).\n", name.c_str(), bytes);
