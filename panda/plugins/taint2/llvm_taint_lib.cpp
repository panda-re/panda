...
void PandaTaintVisitor::insertTaintCompute(Instruction &I, Value *dest, Value *src1, Value *src2, bool is_mixed) {
    ...
    vector<Value *> args{
        llvConst, constSlot(dest), const_uint64(ctx, getValueSize(dest)),
        constSlot(src1), constSlot(src2), const_uint64(ctx, getValueSize(src1)),
        constInstr(&I)
    };
...
void PandaTaintVisitor::visitCmpInst(CmpInst &I) {
    ...
    vector<Value *> cargs{
        llvConst,
        constInstr(&I),
        // Indices of operands in our taint map:
        constSlot(I.getOperand(0)),
