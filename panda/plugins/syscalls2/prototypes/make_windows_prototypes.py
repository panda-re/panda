...
tables = [
    (win2000_x86_syscalls, "windows_2000_x86"), # local import - not supported in volatility 2.6
    (xp_sp2_x86_syscalls, "windows_xpsp2_x86"),
    (xp_sp3_x86_syscalls, "windows_xpsp3_x86"), # local import - not supported in volatility 2.6
...
for syscalls, filename in tables:
    with open('%s_prototypes.txt' % filename, "w") as protofile:
        for i in range(len(syscalls)):
            for j in range(len(syscalls[i])):
                ...
                if name in prototypes:
                    ...
                    print("%d %s %s %s" % (ordinal, ret, name, args), file=protofile)
                else:
                    print("%d missing %s" % (ordinal, name), file=protofile)
