...
struct Profile {
    void         (*enter_switch)(CPUState *, target_ulong, target_ulong, int);
...
void syscall_return(CPUState *cpu, TranslationBlock *tb) {
    ...
    if (likely(ctxi != running_syscalls.end())) {
        ...
        // if we got a return but no syscall
        assert(tb!=NULL);
    }
#if defined(SYSCALL_RETURN_DEBUG)
    if (no >= 0) {
...
void before_tcg_codegen(CPUState *cpu, TranslationBlock *tb){
    ...
    if(res != 0 && res != (target_ulong) -1){
        ...
        if (op!=NULL){
            ...
            LOG_DEBUG("syscalls problem: Couldn't find op to insert return");
            assert("fail"== NULL);
        }
        // insert RETURN
        TCGOp *op2 = find_guest_insn_after_addr(res);
        if (op2 != NULL){
            insert_call(&op2, syscall_return , first_cpu, tb);
        }else{
            LOG_DEBUG("syscalls problem: Couldn't find op to insert return");
            assert("fail"== NULL);
        }
...
void syscall_callback(CPUState *cpu, TranslationBlock *tb, target_ulong pc, int callno) {
    ...
    const syscall_meta_t *sm = syscall_meta;
        ...
        syscalls_profile->enter_switch(cpu, tb->pc, pc, callno);
