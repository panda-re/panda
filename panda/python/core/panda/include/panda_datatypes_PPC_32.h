...
struct TranslationBlock {
	...
	uint8_t *                  llvm_asm_ptr;         /*   160     8 */
	char                       llvm_fn_name[64];     /*   168    64 */
	/* --- cacheline 3 boundary (192) was 40 bytes ago --- */
...
struct ppc_spr_t {
	...
	target_ulong               default_value;        /*    56     4 */
...
struct DisasContext {
	...
	target_ulong               nip;                  /*     8     4 */
	uint32_t                   opcode;               /*    12     4 */
	uint32_t                   exception;            /*    16     4 */
	_Bool                      pr;                   /*    20     1 */
	_Bool                      hv;                   /*    21     1 */
	_Bool                      dr;                   /*    22     1 */
	_Bool                      le_mode;              /*    23     1 */
	_Bool                      lazy_tlb_flush;       /*    24     1 */
	_Bool                      need_access_type;     /*    25     1 */
	/* XXX 2 bytes hole, try to pack */
	int                        mem_idx;              /*    28     4 */
	int                        access_type;          /*    32     4 */
	TCGMemOp                   default_tcg_memop_mask; /*    36     4 */
	_Bool                      fpu_enabled;          /*    40     1 */
	_Bool                      altivec_enabled;      /*    41     1 */
	_Bool                      vsx_enabled;          /*    42     1 */
	_Bool                      spe_enabled;          /*    43     1 */
	_Bool                      tm_enabled;           /*    44     1 */
	/* XXX 3 bytes hole, try to pack */
	ppc_spr_t *                spr_cb;               /*    48     8 */
	int                        singlestep_enabled;   /*    56     4 */
	/* XXX 4 bytes hole, try to pack */
	/* --- cacheline 1 boundary (64 bytes) --- */
	uint64_t                   insns_flags;          /*    64     8 */
	uint64_t                   insns_flags2;         /*    72     8 */
	/* size: 80, cachelines: 2, members: 22 */
	/* sum members: 71, holes: 3, sum holes: 9 */
	/* last cacheline: 16 bytes */
};
struct ppc_tb_t;
...
struct CPUPPCState {
	...
	/* --- cacheline 2 boundary (128 bytes) --- */
	target_ulong               gprh[32];             /*   128   128 */
	/* --- cacheline 4 boundary (256 bytes) --- */
	target_ulong               lr;                   /*   256     4 */
	target_ulong               ctr;                  /*   260     4 */
	uint32_t                   crf[8];               /*   264    32 */
	target_ulong               xer;                  /*   296     4 */
	target_ulong               so;                   /*   300     4 */
	target_ulong               ov;                   /*   304     4 */
	target_ulong               ca;                   /*   308     4 */
	target_ulong               ov32;                 /*   312     4 */
	target_ulong               ca32;                 /*   316     4 */
	/* --- cacheline 5 boundary (320 bytes) --- */
	target_ulong               reserve_addr;         /*   320     4 */
	target_ulong               reserve_val;          /*   324     4 */
	target_ulong               reserve_val2;         /*   328     4 */
	target_ulong               reserve_ea;           /*   332     4 */
	target_ulong               reserve_info;         /*   336     4 */
	target_ulong               msr;                  /*   340     4 */
	target_ulong               tgpr[4];              /*   344    16 */
	float_status               fp_status;            /*   360     8 */
	float64                    fpr[32];              /*   368   256 */
	/* --- cacheline 9 boundary (576 bytes) was 48 bytes ago --- */
	target_ulong               fpscr;                /*   624     4 */
	target_ulong               nip;                  /*   628     4 */
	int                        access_type;          /*   632     4 */
	/* XXX 4 bytes hole, try to pack */
	/* --- cacheline 10 boundary (640 bytes) --- */
	CPUTLBEntry                tlb_table[8][256];    /*   640 65536 */
	/* --- cacheline 1034 boundary (66176 bytes) --- */
	CPUTLBEntry                tlb_v_table[8][8];    /* 66176  2048 */
	/* --- cacheline 1066 boundary (68224 bytes) --- */
	CPUIOTLBEntry              iotlb[8][256];        /* 68224 32768 */
	/* --- cacheline 1578 boundary (100992 bytes) --- */
	CPUIOTLBEntry              iotlb_v[8][8];        /* 100992  1024 */
	/* --- cacheline 1594 boundary (102016 bytes) --- */
	target_ulong               tlb_flush_addr;       /* 102016     4 */
	target_ulong               tlb_flush_mask;       /* 102020     4 */
	target_ulong               vtlb_index;           /* 102024     4 */
	target_ulong               sr[32];               /* 102028   128 */
	/* --- cacheline 1596 boundary (102144 bytes) was 12 bytes ago --- */
	uint32_t                   nb_BATs;              /* 102156     4 */
	target_ulong               DBAT[2][8];           /* 102160    64 */
	/* --- cacheline 1597 boundary (102208 bytes) was 16 bytes ago --- */
	target_ulong               IBAT[2][8];           /* 102224    64 */
	/* --- cacheline 1598 boundary (102272 bytes) was 16 bytes ago --- */
	int32_t                    nb_tlb;               /* 102288     4 */
	int                        tlb_per_way;          /* 102292     4 */
	int                        nb_ways;              /* 102296     4 */
	int                        last_way;             /* 102300     4 */
	int                        id_tlbs;              /* 102304     4 */
	int                        nb_pids;              /* 102308     4 */
	int                        tlb_type;             /* 102312     4 */
	/* XXX 4 bytes hole, try to pack */
	ppc_tlb_t                  tlb;                  /* 102320     8 */
	target_ulong               pb[4];                /* 102328    16 */
	/* --- cacheline 1599 boundary (102336 bytes) was 8 bytes ago --- */
	_Bool                      tlb_dirty;            /* 102344     1 */
	_Bool                      kvm_sw_tlb;           /* 102345     1 */
	/* XXX 2 bytes hole, try to pack */
	uint32_t                   tlb_need_flush;       /* 102348     4 */
	target_ulong               spr[1024];            /* 102352  4096 */
	/* --- cacheline 1663 boundary (106432 bytes) was 16 bytes ago --- */
	ppc_spr_t                  spr_cb[1024];         /* 106448 65536 */
	/* --- cacheline 2687 boundary (171968 bytes) was 16 bytes ago --- */
	ppc_avr_t                  avr[32];              /* 171984   512 */
	/* --- cacheline 2695 boundary (172480 bytes) was 16 bytes ago --- */
	uint32_t                   vscr;                 /* 172496     4 */
	/* XXX 4 bytes hole, try to pack */
	uint64_t                   vsr[32];              /* 172504   256 */
	/* --- cacheline 2699 boundary (172736 bytes) was 24 bytes ago --- */
	uint64_t                   spe_acc;              /* 172760     8 */
	uint32_t                   spe_fscr;             /* 172768     4 */
	float_status               vec_status;           /* 172772     8 */
	/* XXX 4 bytes hole, try to pack */
	ppc_tb_t *                 tb_env;               /* 172784     8 */
	ppc_dcr_t *                dcr_env;              /* 172792     8 */
	/* --- cacheline 2700 boundary (172800 bytes) --- */
	int                        dcache_line_size;     /* 172800     4 */
	int                        icache_line_size;     /* 172804     4 */
	target_ulong               msr_mask;             /* 172808     4 */
	powerpc_mmu_t              mmu_model;            /* 172812     4 */
	powerpc_excp_t             excp_model;           /* 172816     4 */
	powerpc_input_t            bus_model;            /* 172820     4 */
	int                        bfd_mach;             /* 172824     4 */
	uint32_t                   flags;                /* 172828     4 */
	uint64_t                   insns_flags;          /* 172832     8 */
	uint64_t                   insns_flags2;         /* 172840     8 */
	int                        error_code;           /* 172848     4 */
	uint32_t                   pending_interrupts;   /* 172852     4 */
	uint32_t                   irq_input_state;      /* 172856     4 */
	/* XXX 4 bytes hole, try to pack */
	/* --- cacheline 2701 boundary (172864 bytes) --- */
	void * *                   irq_inputs;           /* 172864     8 */
	target_ulong               excp_vectors[99];     /* 172872   396 */
	/* --- cacheline 2707 boundary (173248 bytes) was 20 bytes ago --- */
	target_ulong               excp_prefix;          /* 173268     4 */
	target_ulong               ivor_mask;            /* 173272     4 */
	target_ulong               ivpr_mask;            /* 173276     4 */
	target_ulong               hreset_vector;        /* 173280     4 */
	/* XXX 4 bytes hole, try to pack */
	hwaddr                     mpic_iack;            /* 173288     8 */
	_Bool                      mpic_proxy;           /* 173296     1 */
	_Bool                      has_hv_mode;          /* 173297     1 */
	_Bool                      in_pm_state;          /* 173298     1 */
	/* XXX 5 bytes hole, try to pack */
	opc_handler_t *            opcodes[64];          /* 173304   512 */
	/* --- cacheline 2715 boundary (173760 bytes) was 56 bytes ago --- */
	target_ulong               hflags;               /* 173816     4 */
	target_ulong               hflags_nmsr;          /* 173820     4 */
	/* --- cacheline 2716 boundary (173824 bytes) --- */
	int                        immu_idx;             /* 173824     4 */
	int                        dmmu_idx;             /* 173828     4 */
	int                        (*check_pow)(CPUPPCState *); /* 173832     8 */
	void *                     load_info;            /* 173840     8 */
	uint8_t                    fit_period[4];        /* 173848     4 */
	uint8_t                    wdt_period[4];        /* 173852     4 */
	target_ulong               tm_gpr[32];           /* 173856   128 */
	/* --- cacheline 2718 boundary (173952 bytes) was 32 bytes ago --- */
	ppc_avr_t                  tm_vsr[64];           /* 173984  1024 */
	/* --- cacheline 2734 boundary (174976 bytes) was 32 bytes ago --- */
	uint64_t                   tm_cr;                /* 175008     8 */
	uint64_t                   tm_lr;                /* 175016     8 */
	uint64_t                   tm_ctr;               /* 175024     8 */
	uint64_t                   tm_fpscr;             /* 175032     8 */
	/* --- cacheline 2735 boundary (175040 bytes) --- */
	uint64_t                   tm_amr;               /* 175040     8 */
	uint64_t                   tm_ppr;               /* 175048     8 */
	uint64_t                   tm_vrsave;            /* 175056     8 */
	uint32_t                   tm_vscr;              /* 175064     4 */
	/* XXX 4 bytes hole, try to pack */
	uint64_t                   tm_dscr;              /* 175072     8 */
	uint64_t                   tm_tar;               /* 175080     8 */
	/* size: 175088, cachelines: 2736, members: 100 */
	/* sum members: 175053, holes: 9, sum holes: 35 */
	/* last cacheline: 48 bytes */
};
struct CPUState;
typedef struct CPUState CPUState;
