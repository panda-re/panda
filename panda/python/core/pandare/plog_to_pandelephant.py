...
def plogDispatch(pandalog, skip_steps, collectFrom, collectTypes=False):
        '''
...
def initialCollection(pandalog, skip_steps=None):
    ...
    def _create_thread_procs_and_syscalls_from_syscall(entry, msg):
        nonlocal threads, processes, collectedSyscalls
        SyscallFieldInfo = {
            "str": ("string", "{:s}"),
            "ptr": ("pointer", "0x{:x}"),
            "u64": ("unsigned64", "{:d}"),
        ...
        # First update threads and processes with info from this syscall
        thread = CollectedThread(
            ProcessId=msg.pid,
            ParentProcessId=msg.ppid,
            ThreadId=msg.tid,
        ...
        # Then store info on this syscall in collectedSyscalls
        collectedSyscalls.add(
            CollectedSyscall(
                Name=msg.call_name,
                RetVal=msg.retcode,
                Thread=thread,
    ...
    def _create_thread_procs_from_proctrace(entry, msg):
        nonlocal threads, processes, thread_slices, last_thread_info
        thread = CollectedThread(
            ProcessId=msg.pid,
            ParentProcessId=msg.ppid,
            ThreadId=msg.tid,
    ...
    types = plogDispatch(pandalog, skip_steps, {
    ...
    print(f"Gathered {len(processes)} Processes, {len(threads)} Threads"\
          f" and {len(collectedSyscalls)} syscalls")

    return processes, threads, thread_slices, thread_names, collectedSyscalls, types
...
def collectProcessMemoryMappings(pandalog, processes, skip_steps):
    ...
    def _collect_memmaps_from_asidlib(entry, msg):
        nonlocal collectedBetterMappings, collectedBetterMappingRanges, num_no_mappings
        ...
        for mapping in msg.modules:
            ...
            if better_mapping in collectedBetterMappingRanges[process].keys():
                ...
                ) = collectedBetterMappingRanges[process][better_mapping]
    ...
    plogDispatch(pandalog, skip_steps, {
        "asid_libraries": _collect_memmaps_from_asidlib,
    })

    print(f"{num_no_mappings} messages processed without mappings")

    mapping_len = sum([len(procmaps.keys())
                       for procmaps in collectedBetterMappingRanges.values()
                     ])
    print(f"collectedBetterMappingRanges Len {mapping_len}")

    return collectedBetterMappings, collectedBetterMappingRanges
...
def collectTaintFlows():
    ...
    def _collect_taint_flows(entry, msg):
        nonlocal collectedCodePoints, collectedTaintFlows, noMappingCount
        source_thread = msg.source.cp.thread
        ...
        for mapping, (
            FirstInstructionCount,
            LastInstructionCount,
        ) in collectedBetterMappingRanges[sink_analyze].items():
        ...
        for mapping, (
            FirstInstructionCount,
            LastInstructionCount,
        ) in collectedBetterMappingRanges[source_analyze].items():
    ...
    print(
        "Collected {} Taint Flows {} Syscalls {} Code Points".format(
            len(collectedTaintFlows),
            len(collectedSyscalls),
            len(collectedCodePoints),
        )
    )

    return collectedCodePoints, collectedTaintFlows
...
def PLogToPandelephant(pandalog, db_url, exec_name, skip_steps=None, verbose=False):
    ...
    if "taint_flow" in plog_types:
        ...
        collectedCodePoints, collectedTaintFlows = collectTaintFlows(pandalog, skip_steps)
    else:
        collectedCodePoints = set()
        collectedTaintFlows = set()
