...
These files are subsequently processed by the `protoc-c` compiler to generate two
`plog.pb-c.[ch]` files.
...
In particular, you will probably want to consult the `plog.pb-c.h` header,
since it defines the logging struct `Panda__LogEntry`.
...
There is a small program in `panda/src/plog_reader.cpp`.
